...
declare global {
  interface Window {
    IMP: {
      ...
      request_pay: (request: PortoneRequest, callback: (response: PortoneResponse) => void) => void;
    };
  }
}
  ...
  const handleTestPayment = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/payment/verify', {
        ...
        body: JSON.stringify({ imp_uid: 'mock_uid_' + Date.now(), merchant_uid }),
      });
      ...
      const message = err instanceof Error ? err.message : '테스트 결제 중 오류가 발생했습니다.';
      toast.error(message);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="container py-20 space-y-12">
      <div className="text-center space-y-4">
        <h1 className="text-4xl md:text-6xl font-bold font-display tracking-tight text-primary">Unlock Your Full Potential</h1>
      ...
      <div className="flex flex-col items-center gap-8">
        ...
        {/* Dev Only Button */}
        <Button variant="ghost" onClick={handleTestPayment} disabled={isLoading} className="text-muted-foreground text-xs underline">
          [개발자 전용] 결제 없이 테스트하기
        </Button>
      </div>
    </div>
