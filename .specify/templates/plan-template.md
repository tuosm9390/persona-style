...
- [ ] **이미지 품질 보장**: 생성 시간이 3초 이내이며 고해상도 출력이 고려되었는가?
- [ ] **데이터 프라이버시**: 분석 데이터의 익명화 및 RLS 접근 제어가 설계되었는가?
- [ ] **리텐션/커뮤니티**: 시계열 추적 가치 및 익명 공유 안전 장치가 고려되었는가?
- [ ] **결제 및 프리미엄**: 결제 프로세스의 보안과 리포트 품질이 보장되었는가?
- [ ] **아키텍처 준수**: 기능 기반 구조(`src/features`) 및 서비스 레이어 분리가 설계에 반영되었는가?
